  ...
  return (
    ...
    {isLogin ? (<CS.FactsContainer>
      ...
      {data?.data?.length !== 0 && facts === 4 && (
        <CS.FactTitle>
          {t("interestingFacts.totalNumber", { count: data?.data.totalNumberOfPeopel})}
        </CS.FactTitle>
      )}
    </CS.FactsContainer>) : 
     <CS.FactsContainer>
      <CS.FactTitle>{t("interestingFacts.interestingFactsNoLogin")}</CS.FactTitle>
    </CS.FactsContainer>}
    </>
  );
};
